...
export function LoginMenu() {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      // ── Mock fallback (API not reachable) ─────────────────────────────────
      const foundUser = initialEmployees.find(emp => emp.email === email);
      ...
      // Derive API-style role from the mock role string
      ...
      navigate(roleToRoute(roleMap[foundUser.role] ?? ''));
    } finally {
      setLoading(false);
    }
  };
